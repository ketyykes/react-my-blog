...
換句話說，當業界選擇用 `harness` 來描述新的工程典範時，並不是在隨意挪用名詞，而是非常準確地點出了我們此刻面對 LLM 的處境 — **模型的能力足夠強大，但需要外圍的約束才跑得穩、跑得久**。
...
一句話總結 — context engineering **管的是推論時所有進入 context window 的資訊**。它透過外部記憶的導入與裁剪，控制每一輪推論時模型該知道、不該知道的事情；解決的問題從「該怎麼問」進化到「該讓模型知道什麼」。
...
這些問題不是靠寫更好的 prompt 或塞更多 context 能解決的，而是需要**模型之外**的機制 — 例如把規劃、執行、評估拆給不同角色的 agent；例如在每個 feature 完成後強制 git commit；例如建立明確的「完成定義」合約。

一句話總結 — harness engineering 把焦點放到**整個 agent runtime 與驗證治理系統**，它解決的問題是「整個系統該怎麼運作才能可靠」。
...
**第三組：產業實例 — Stripe**。Stripe 工程團隊每週合併**約 1,300 個 AI 生成的 PR**，這個量級背後仰賴的不是 prompt 技巧，而是一套高度成熟的 harness 系統：linter 強制阻擋不合規的 PR、結構化的 spec、跨多會話的進度追蹤、自動回滾機制。

由於這些數據幾乎都來自業界第一線的真實場景，因此 harness engineering 不再只是學術名詞，而是**直接決定 AI 投資能否落地的工程能力**。
...
由於 agent 撰寫程式碼的速度實在太快了，**審查反而成為人類的新瓶頸**；因此如果我們把人類擺在每一個節點，整體的產出效率會直接被人類的注意力上限拉低。換句話說，harness engineering 的人類檢查點不是「越多越好」，而是「越準越好」 — 我們應該把人類保留在**需求定義、風險判斷、評測校準與規則設計**這四件高價值的事情上，把低價值的重複性操作交給 harness 自動化。
...
- **Harness engineering** 管的是整個 agent runtime 與驗證治理 — 我們在意「整個系統該怎麼運作」

三者不是競爭，而是巢狀的關係。即使未來模型能力再強大，harness 也不會消失，它只會**演化** — 漸進簡化掉非必要組件、保留真正能補足模型限制的部分。
